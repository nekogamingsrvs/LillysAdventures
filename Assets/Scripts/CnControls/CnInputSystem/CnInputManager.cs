...
namespace CnControls
{
	...
	public class CnInputManager
	{
		...
		/// <summary>
		/// Additional logic for touch retrieval
		...
		{
			// If we have the axis registered as virtual, we call the retrieval logic
			if (AxisExists(axisName))
			{
				return GetVirtualAxisValue(Instance._virtualAxisDictionary[axisName], axisName, isRaw);
			}

			// If we don't have the desired virtual axis registered, we just fallback to the default Unity Input behavior
