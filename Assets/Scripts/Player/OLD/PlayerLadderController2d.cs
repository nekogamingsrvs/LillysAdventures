...
/*
/// <summary>
/// Controls and animates the player while it is on a ladder.
/// </summary>
class PlayerLadderController2d : PlayerController2d
{
	...
	protected override void OnUpdate()
	{
		...
		{
			climbing = Input.GetAxisRaw("Vertical");
			jumping = Input.GetButtonDown("Jump");
        }
		else if (Application.platform == RuntimePlatform.Android)
		{
			climbing = CnInputManager.GetAxisRaw("Vertical");
			jumping = CnInputManager.GetButtonDown("Jump");
		}

		this.Animator.speed = Mathf.Abs(climbing);

		// Are we jumping? If so, we leave this state.
		if (climbing == 0 && jumping)
		{
		...
		if (climbing < 0)
		{
			...
			RaycastHit2D ladderBottomRay = Physics2D.Linecast(this.Linecast1.position, this.transform.position, 1 << LayerMask.NameToLayer("LadderBottoms"));
