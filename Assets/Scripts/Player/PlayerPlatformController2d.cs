...
class PlayerPlatformController2d : PlayerController2d
{
	...
	protected override void OnUpdate()
	{
		...
		{
			jumping = CnInputManager.GetButton("Jump");
		}

		if (this.Controller2d.isGrounded && jumping)
		{
			// Jumping
	...
	private void UpdateHorizontalInput()
	{
		...
		if (Application.platform == RuntimePlatform.WindowsEditor)
		{
			...
			running = Input.GetButton("RunKey");
		}
		else if (Application.platform == RuntimePlatform.Android)
		{
		...
		if (moving != 0)
		{
			...
			if (this.Master.FaceDirection(moving))
			{
				moving = 0;
		...
		if (running)
		{
			this.velocity.x = moving * modifier * 2;
			this.Animator.SetBool("Running", true);
		}
		else
		{
			this.velocity.x = moving * modifier;
